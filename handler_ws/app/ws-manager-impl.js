...
// Session strategies
const SESSION_ISOLATED = 1;      // Each tab gets its own session
const SESSION_SHARED = 2;        // Tabs share session, but have separate connections
const SESSION_SHARED_CONNECTION = 4; // Tabs share session AND connection (SharedWorker)
...
const COORD_SESSION_UPDATE = 1024;
...
class WebSocketManager {
    ...
    constructor(options = {}) {
        if (!options.wsRoute || !options.wsWorkerRoute || !options.endpoint) {
            throw new Error('wsRoute, wsWorkerRoute, and endpoint options are required');
        }
        this.#options = Object.assign({
            debug: false,
            wsRoute: options.wsRoute,
            workerRoute: options.wsWorkerRoute,
            autoConnect: true,
            reconnectOnDisconnect: true,
            maxReconnectAttempts: 10,
            endpoint: options.endpoint, // Default endpoint, used for namespacing
            sessionStrategy: SESSION_SHARED, // Default: shared session, separate connections
            connIdStorageKey: 'ws-conn-id',
            sessionIdStorageKey: 'ws-session-id',
        ...
        // Normalize endpoint for use in storage keys (replace slashes with dashes)
        this.#options.endpointKey = this.#options.endpoint.replace(/\//g, '-');
        
        // Public properties
        this.connectionId = this.#getStoredConnectionId();
        this.sessionId = this.#getStoredSessionId();
        this.sessionData = this.#loadSessionData();
        this.sessionStrategy = this.#options.sessionStrategy;
        const storedMode = localStorage.getItem(this.#getStorageKey('modePref'));
        this.connectionMode = storedMode ? parseInt(storedMode, 10) : null;
    ...
    setPreferredMode(mode) {
        if ([MODE_WORKER, MODE_DIRECT].includes(mode)) {
            ...
            localStorage.setItem(this.#getStorageKey('modePref'), mode);
     ...
     */
    clearPreferredMode() {
        localStorage.removeItem(this.#getStorageKey('modePref'));
    ...
    connectViaSharedWorker() {
        ...
        // SharedWorker uses SESSION_SHARED_CONNECTION strategy
        this.sessionStrategy = SESSION_SHARED_CONNECTION;
        ...
        try {
            ...
            this.#worker = new SharedWorker(this.#options.workerRoute);
    ...
    connectDirectly() {
        ...
        // Direct mode explicitly does NOT use coordination or BroadcastChannel
        // Each tab is independent
        if (this.broadcastChannel) {
            console.log('[WS] Direct mode: Closing BroadcastChannel');
            try {
        ...
        const url = `${protocol}//${window.location.host}${this.#options.wsRoute}?connid=${tabSpecificId}&sessionid=${this.sessionId}`;
    ...
    #handleCoordinationMessage(data) {
        try {
            ...
            switch (data.type) {
                ...
                case COORD_SESSION_UPDATE:
                    // Sync session data from another tab (only for shared sessions)
                    if (this.#options.sessionStrategy === SESSION_SHARED && data.id !== this.#coordinationId) {
                        this.sessionData = data.sessionData;
                        console.log('[WS] Session data synced from another tab');
                    }
                    break;
     ...
     */
    #getStorageKey(type) {
        const baseKey = this.#options[type + 'StorageKey'];
        return `${baseKey}${this.#options.endpointKey}`;
    }
     ...
     */
    #getStoredSessionId() {
        // SESSION_ISOLATED: Each tab gets unique session
        if (this.#options.sessionStrategy === SESSION_ISOLATED) {
            return this.#generateInstanceId();
        }
        
        // SESSION_SHARED or SESSION_SHARED_CONNECTION: Shared session across tabs
    ...
    /**
     * Reset the session ID to a new value
     */
    #resetSessionId() {
        this.sessionId = this.#generateConnectionId();
        localStorage.setItem(this.#getStorageKey('sessionId'), this.sessionId);
        this.sessionData = {};
        return this.sessionId;
    }

    /**
     * Load session data from localStorage (for shared sessions)
     */
    #loadSessionData() {
        if (this.#options.sessionStrategy === SESSION_ISOLATED) {
            return {};
        ...
        const key = this.#getStorageKey('sessionData');
        const data = localStorage.getItem(key);
        return data ? JSON.parse(data) : {};
    }

    /**
     * Save session data to localStorage (for shared sessions)
     */
    #saveSessionData() {
        if (this.#options.sessionStrategy === SESSION_ISOLATED) {
            return;
    ...
    setSessionValue(key, value) {
        ...
        console.log(`[WS] Session[${key}] = ${JSON.stringify(value)}`);
    }
    ...
    #getStoredConnectionId() {
        const key = this.#getStorageKey('connId');
        let id = localStorage.getItem(key);
        if (!id) {
            id = this.#generateConnectionId();
            localStorage.setItem(key, id);
    ...
    #resetConnectionId() {
        ...
        localStorage.setItem(this.#getStorageKey('connId'), this.connectionId);
    ...
    #clearSharedWorkers() {
        ...
        try {
            const tempWorker = new SharedWorker(this.#options.workerRoute);
        ...
        try {
            const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
            const url = `${protocol}//${window.location.host}${this.#options.wsRoute}?connid=${this.connectionId}&cleanup=1`;
...
export async function createWebSocketManager(options = {}) {
    return new Promise((resolve) => {
        ...
        const initManager = () => {
            ...
            // Only initialize coordination if using SharedWorker mode
            // Direct mode connections should NOT have coordination
            if (manager.connectionMode === MODE_WORKER || options.enableCoordination === true) {
