var W=window.location.pathname.split("/").filter((q)=>q),B=W.length>0?"/"+W[0]:"";class D{constructor(){let q=window.location.pathname.split("/").filter((J)=>J),G=q.length>0?"/"+q[0]:"";this.baseUrl=`${G}/trail`,this.loading=!1,this.error=null}async fetchServerList(){this.loading=!0,this.error=null;try{let q=await fetch(`${this.baseUrl}/servers/a/list`);if(!q.ok)throw Error(`HTTP error! status: ${q.status}`);let G=await q.json();return this.loading=!1,G}catch(q){throw this.loading=!1,this.error=q.message,q}}async fetchServerDetails(q){this.loading=!0,this.error=null;try{let G=await fetch(`${this.baseUrl}/servers/i/${q}/details`);if(!G.ok)throw Error(`HTTP error! status: ${G.status}`);let J=await G.json();return this.loading=!1,J}catch(G){throw this.loading=!1,this.error=G.message,G}}async fetchFilters(){this.loading=!0,this.error=null;try{let q=await fetch(`${this.baseUrl}/servers/a/filters`);if(!q.ok)throw Error(`HTTP error! status: ${q.status}`);let G=await q.json();return this.loading=!1,G}catch(q){throw this.loading=!1,this.error=q.message,q}}}function x(){return new D}var A=null,E=[],_=[],Z=1,R=20,V=null,X={search:"",region:"",zone:"",state:"",type:""},L={};function K(q){return q.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function j(q){let G={};for(let J in L)G[J]=q[L[J]];return G}function w(){document.getElementById("totalServers").textContent=E.length,document.getElementById("displayedServers").textContent=_.length}function O(q){return{running:'<span class="status-badge status-running">● Running</span>',stopped:'<span class="status-badge status-stopped">● Stopped</span>',pending:'<span class="status-badge status-pending">● Pending</span>',stopping:'<span class="status-badge status-stopping">● Stopping</span>',terminated:'<span class="status-badge status-terminated">● Terminated</span>'}[q]||`<span class="status-badge">${K(q)}</span>`}function u(q){if(!q.CPUCores||!q.RAMTotalGB)return"-";return`${q.CPUCores} vCPU, ${q.RAMTotalGB} GB RAM`}function M(q){if(!q)return"-";return new Date(q).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}function y(q){let G=(J)=>typeof q==="object"&&!Array.isArray(q)?q[J]:q[L[J]];return`
    ...
    `}function S(){let q=Math.ceil(_.length/R),G=document.getElementById("pagination");if(q<=1){G.innerHTML="";return}let J="";if(Z>1)J+='<button onclick="window.goToPage('+(Z-1)+')">← Previous</button>';let Q=7,Y=Math.max(1,Z-Math.floor(Q/2)),U=Math.min(q,Y+Q-1);if(U-Y<Q-1)Y=Math.max(1,U-Q+1);if(Y>1){if(J+='<button onclick="window.goToPage(1)">1</button>',Y>2)J+="<span>...</span>"}for(let N=Y;N<=U;N++)J+='<button class="'+(N===Z?"active":"")+'" onclick="window.goToPage('+N+')">'+N+"</button>";if(U<q){if(U<q-1)J+="<span>...</span>";J+='<button onclick="window.goToPage('+q+')">'+q+"</button>"}if(Z<q)J+='<button onclick="window.goToPage('+(Z+1)+')">Next →</button>';G.innerHTML=J}function T(){let q=document.getElementById("instancesTableBody"),G=(Z-1)*R,J=G+R,Q=_.slice(G,J);if(Q.length===0){q.innerHTML='<tr class="empty-row"><td colspan="7" class="empty-state">No instances found</td></tr>';return}q.innerHTML=Q.map(y).join(""),S(),f()}function b(q){X.search=q||"",$()}function $(){_=E.filter((q)=>{let G=Array.isArray(q)?j(q):q;if(X.search){let J=X.search.toLowerCase();if(!(G.ID.toLowerCase().includes(J)||G.Hostname.toLowerCase().includes(J)||G.PublicIPv4&&G.PublicIPv4.toLowerCase().includes(J)||G.PrivateIPv4&&G.PrivateIPv4.toLowerCase().includes(J)))return!1}if(X.region&&G.Region!==X.region)return!1;if(X.zone&&G.Zone!==X.zone)return!1;if(X.state&&G.State!==X.state)return!1;if(X.type){if(u(G)!==X.type)return!1}return!0}),Z=1,w(),T()}function F(q){V=q;let G=document.getElementById("regionFilter");q.regions.forEach((U)=>{let N=document.createElement("option");N.value=U,N.textContent=U,G.appendChild(N)});let J=document.getElementById("zoneFilter");q.zones.forEach((U)=>{let N=document.createElement("option");N.value=U,N.textContent=U,J.appendChild(N)});let Q=document.getElementById("stateFilter");q.states.forEach((U)=>{let N=document.createElement("option");N.value=U,N.textContent=U.charAt(0).toUpperCase()+U.slice(1),Q.appendChild(N)});let Y=document.getElementById("typeFilter");q.instanceTypes.forEach((U)=>{let N=document.createElement("option");N.value=U,N.textContent=U,Y.appendChild(N)})}function f(){document.querySelectorAll(".clickable-row").forEach((G)=>{G.addEventListener("click",async()=>{let J=G.getAttribute("data-instance-id");await k(J)})})}async function k(q){let G=document.getElementById("detailsModal"),J=document.getElementById("modalContent");G.style.display="flex",J.innerHTML='<div class="modal-loader">Loading instance details...</div>';try{let Q=await A.fetchServerDetails(q);J.innerHTML=H(Q)}catch(Q){J.innerHTML=`<div class="error-state">Failed to load details: ${K(Q.message)}</div>`}}function H(q){let G=`
    ...
    `;if(q.ServerInfo)G+=`
        ...
        `;return G}function h(){let q=document.getElementById("detailsModal");q.style.display="none"}function C(q){let G=document.getElementById("refreshBtn"),J=document.getElementById("loader"),Q=document.querySelector(".table-container");if(q){if(G.disabled=!0,J.style.display="block",Q)Q.style.opacity="0.5"}else if(G.disabled=!1,J.style.display="none",Q)Q.style.opacity="1"}function P(q){let G=document.getElementById("instancesTableBody");G.innerHTML=`<tr class="empty-row"><td colspan="7"><div class="error-state">❌ Error: ${K(q)}</div></td></tr>`}async function z(){C(!0);try{let q=await A.fetchServerList();if(q.columns&&q.data)L={},q.columns.forEach((J,Q)=>{L[J]=Q}),E=q.data;else E=q||[];_=[...E],w(),T();let G=new Date().toLocaleTimeString();if(document.getElementById("lastUpdate").textContent=G,!V){let J=await A.fetchFilters();F(J)}}catch(q){console.error("Failed to load servers:",q),P(q.message)}finally{C(!1)}}window.goToPage=function(q){Z=q,T(),window.scrollTo({top:0,behavior:"smooth"})};function I(){A=x();let q=document.getElementById("refreshBtn");if(q)q.addEventListener("click",z);let G=document.getElementById("searchInput");if(G)G.addEventListener("input",(N)=>{b(N.target.value)});let J=document.getElementById("regionFilter");if(J)J.addEventListener("change",(N)=>{X.region=N.target.value,$()});let Q=document.getElementById("zoneFilter");if(Q)Q.addEventListener("change",(N)=>{X.zone=N.target.value,$()});let Y=document.getElementById("stateFilter");if(Y)Y.addEventListener("change",(N)=>{X.state=N.target.value,$()});let U=document.getElementById("typeFilter");if(U)U.addEventListener("change",(N)=>{X.type=N.target.value,$()});z()}if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",I);else I();window.closeModal=h;
